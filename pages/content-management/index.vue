    ...
    <div class="w-full relative mt-12 mx-4" style="max-width: 1150px;">
      ...
      <div
        ...
        <div class="w-full flex justify-center">
          <span class="flex justify-center items-center w-8 h-8 rounded shadow text-gray-100 bg-gray-700 select-none cursor-pointer active:bg-gray-800 " @click="$router.push({path: $route.path, query: {page: `${Number($route.query.page) - 1}` }})">-</span>
          <div class="flex mx-1">
            <span v-for="i in 5" class="flex mx-0.5 justify-center items-center w-8 h-8 rounded shadow text-blue-100 bg-blue-800 select-none cursor-pointer active:bg-blue-900 opacity-50" :class="{'opacity-100' : i === +$route.query.page}" @click="$router.push({path: $route.path, query: {page: `${i}` }})">{{ i }}</span>
          </div>
          <span class="flex justify-center items-center w-8 h-8 rounded shadow text-gray-100 bg-gray-700 select-none cursor-pointer active:bg-gray-800 " @click="$router.push({path: $route.path, query: {page: `${Number($route.query.page) + 1}` }})">+</span>
        </div>
